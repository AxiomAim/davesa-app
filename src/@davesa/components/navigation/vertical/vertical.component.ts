...
import {
    ...
    DavesaVerticalNavigationAppearance,
    DavesaVerticalNavigationMode,
    DavesaVerticalNavigationPosition,
} from '@davesa/components/navigation/navigation.types';
import { DavesaVerticalNavigationAsideItemComponent } from '@davesa/components/navigation/vertical/components/aside/aside.component';
...
import { DavesaVerticalNavigationGroupItemComponent } from '@davesa/components/navigation/vertical/components/group/group.component';
...
import { DavesaScrollbarDirective } from '@davesa/directives/scrollbar/scrollbar.directive';
...
@Component({
    selector: 'davesa-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: davesaAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'davesaVerticalNavigation',
    standalone: true,
    imports: [
        DavesaScrollbarDirective,
        DavesaVerticalNavigationAsideItemComponent,
        ...
        DavesaVerticalNavigationGroupItemComponent,
        DavesaVerticalNavigationSpacerItemComponent,
    ],
})
export class DavesaVerticalNavigationComponent
    implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: DavesaVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: DavesaVerticalNavigationMode = 'side';
    @Input() name: string = this._davesaUtilsService.randomId();
    @Input() navigation: DavesaNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: DavesaVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output()
    readonly appearanceChanged: EventEmitter<DavesaVerticalNavigationAppearance> =
        new EventEmitter<DavesaVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<DavesaVerticalNavigationMode> =
        new EventEmitter<DavesaVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output()
    readonly positionChanged: EventEmitter<DavesaVerticalNavigationPosition> =
        new EventEmitter<DavesaVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<DavesaNavigationItem> =
        new ReplaySubject<DavesaNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<DavesaNavigationItem> =
        new ReplaySubject<DavesaNavigationItem>(1);
    ...
    private _davesaScrollbarDirectives!: QueryList<DavesaScrollbarDirective>;
    private _davesaScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'davesa-vertical-navigation-animations-enabled':
                this._animationsEnabled,
            [`davesa-vertical-navigation-appearance-${this.appearance}`]: true,
            'davesa-vertical-navigation-hover': this._hovered,
            'davesa-vertical-navigation-inner': this.inner,
            'davesa-vertical-navigation-mode-over': this.mode === 'over',
            'davesa-vertical-navigation-mode-side': this.mode === 'side',
            'davesa-vertical-navigation-opened': this.opened,
            'davesa-vertical-navigation-position-left': this.position === 'left',
    ...
    /**
     * Setter for davesaScrollbarDirectives
     */
    @ViewChildren(DavesaScrollbarDirective)
    set davesaScrollbarDirectives(
        davesaScrollbarDirectives: QueryList<DavesaScrollbarDirective>
    ) {
        // Store the directives
        this._davesaScrollbarDirectives = davesaScrollbarDirectives;

        // Return if there are no directives
        if (davesaScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._davesaScrollbarDirectivesSubscription) {
            this._davesaScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._davesaScrollbarDirectivesSubscription = merge(
            ...
            .subscribe(() => {
                ...
                davesaScrollbarDirectives.forEach((davesaScrollbarDirective) => {
                    davesaScrollbarDirective.update();
                });
            });
    }
    ...
    ngAfterViewInit(): void {
        ...
        setTimeout(() => {
            ...
            if (
                ...
                const activeItem =
                    this._navigationContentEl.nativeElement.querySelector(
                        '.davesa-vertical-navigation-item-active'
                    );
            ...
            // Otherwise
                ...
                this._davesaScrollbarDirectives.forEach(
                    (davesaScrollbarDirective) => {
                        // Skip if not enabled
                        if (!davesaScrollbarDirective.isEnabled()) {
                            return;
                        }

                        // Scroll to the active element
                        davesaScrollbarDirective.scrollToElement(
                            '.davesa-vertical-navigation-item-active',
                            -120,
                            true
                        );
     ...
     * @param item
     */
    openAside(item: DavesaNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: DavesaNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('davesa-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add(
                'davesa-vertical-navigation-overlay-transparent'
            );
        }
    ...
    private _showAsideOverlay(): void {
        ...
        this._asideOverlay.classList.add(
            'davesa-vertical-navigation-aside-overlay'
        );
