...
import {
    ScrollbarGeometry,
    ScrollbarPosition,
} from '@davesa/directives/scrollbar/scrollbar.types';
 ...
 */
@Directive({
    selector: '[davesaScrollbar]',
    exportAs: 'davesaScrollbar',
    standalone: true,
})
export class DavesaScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_davesaScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    private _elementRef = inject(ElementRef);
    private _platform = inject(Platform);

    @Input() davesaScrollbar: boolean = true;
    @Input() davesaScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('davesaScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.davesaScrollbar = coerceBooleanProperty(
                changes.davesaScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.davesaScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('davesaScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.davesaScrollbarOptions.currentValue
            );
