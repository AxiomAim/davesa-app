...
import {
    cros as crosData,
} from 'app/mock-api/apps/cros/data';
import { assign, cloneDeep } from 'lodash-es';
import { from, map } from 'rxjs';

@Injectable({ providedIn: 'root' })
export class CrosMockApi {
    private _cros: any[] = crosData;
    ...
    registerHandlers(): void {
        ...
        // @ Cros - GET
        // -----------------------------------------------------------------------------------------------------
        this._davesaMockApiService.onGet('api/apps/cros/all').reply(() => {
            // Clone the cros
            const cros = cloneDeep(this._cros);
        ...
        this._davesaMockApiService
            ...
            .reply(({ request }) => {
                ...
                this._cros.forEach((item, index, cros) => {
                    if (item.id === id) {
                        // Update the cro
        ...
        this._davesaMockApiService
            ...
            .reply(({ request }) => {
                ...
                this._cros.forEach((item, index) => {
                    if (item.id === id) {
                        this._cros.splice(index, 1);
                    }
                });
