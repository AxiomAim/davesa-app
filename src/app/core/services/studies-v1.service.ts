...
export class StudyV1Service {
  ...
  getStudyList(params: FilterParams): Observable<List<Study>> {
    return this.http.post<List<Study>>("study", params);
  }

  getStudyById(studyId: string): Observable<GetOne<Study>> {
    return this.http.get<GetOne<Study>>("study/get-one-study/" + studyId);
  }

  createStudy(data): Observable<any> {
    return this.http.post("study/create", data);
  }

  updateStudy(study: any): Observable<any> {
    return this.http.put("study/update/" + study.oid, study);
  }
  ...
  cloneStudy(data): Observable<any> {
    return this.http.post("study/clone-study", data);
  }
  ...
  getChatStatusLogs(id: string, data: FilterParams): Observable<List<ChatStatusLog>> {
    return this.http.post<List<ChatStatusLog>>(`chat/get-status-chat-logs/${id}`, data);
  }
  ...
  deleteCtaDocument(oid):Observable<any>{
    return this.http.delete('study/delete-upload-study-clinical-trial-document/delete/'+oid);
  }
