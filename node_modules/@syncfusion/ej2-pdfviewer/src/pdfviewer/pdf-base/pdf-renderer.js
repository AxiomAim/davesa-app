...
import { DataFormat, PdfAnnotationExportSettings, PdfDocument, PdfRotationAngle, PdfTextStyle, PdfDocumentLinkAnnotation, PdfTextWebLinkAnnotation, PdfUriAnnotation, PdfPermissionFlag, PdfFormFieldExportSettings, _bytesToString, _encode, PdfPageSettings, PdfSignatureField } from '@syncfusion/ej2-pdf';
...
var PdfRenderer = /** @class */ (function () {
    ...
    PdfRenderer.prototype.rotatePages = function (jsonObject) {
        if (jsonObject.hasOwnProperty('organizePages')) {
            ...
            for (var i = 0; i < organizePages.length; i++) {
                ...
                if (!isNullOrUndefined(currentPageNumber) && !isNullOrUndefined(rotation) && !isDeleted && !isInserted && pageNumber !== -1) {
                    var page = this.loadedDocument.getPage(currentPageNumber);
                    page.rotation = this.getPdfRotationAngle(rotation);
                }
            }
        }
    };
    PdfRenderer.prototype.insertPdfPages = function (jsonObject) {
        if (jsonObject.hasOwnProperty('organizePages')) {
            var organizePages = JSON.parse(jsonObject.organizePages);
            for (var i = 0; i < organizePages.length; i++) {
                var pageNumber = organizePages[parseInt(i.toString(), 10)].pageIndex;
    ...
    PdfRenderer.prototype.deletePdfPages = function (jsonObject) {
        if (jsonObject.hasOwnProperty('organizePages')) {
            var organizePages = JSON.parse(jsonObject.organizePages);
            var deleteCount = 0;
    ...
    // eslint-disable-next-line max-len
    PdfRenderer.prototype.textExtraction = function (pageIndex, isLayout, isRenderText, jsonObject, requestType, annotationObject) {
        var _this = this;
        this.documentTextCollection = [];
        return new Promise(function (resolve, reject) {
            if (!isNullOrUndefined(_this.pdfViewerBase.pdfViewerRunner)) {
                _this.pdfViewerBase.pdfViewerRunner.postMessage({ pageIndex: pageIndex, message: 'extractText', zoomFactor: _this.pdfViewer.magnificationModule.zoomFactor, isTextNeed: true, isRenderText: isRenderText, jsonObject: jsonObject, requestType: requestType, annotationObject: annotationObject });
            }
            else {
                resolve(null);
            }
     ...
     * @private
     */
    PdfRenderer.prototype.textExtractionOnmessage = function (event) {
        var extractedText = '';
        ...
        if (event.data.message === 'textExtracted') {
            ...
            for (var i = 0; i < characterDetails.length; i++) {
                ...
                // eslint-disable-next-line max-len
                var bound = new AnnotBounds(this.convertPixelToPoint(characterDetails[parseInt(i.toString(), 10)].X), this.convertPixelToPoint(characterDetails[parseInt(i.toString(), 10)].Y + cropBox[1]), this.convertPixelToPoint(characterDetails[parseInt(i.toString(), 10)].Width), this.convertPixelToPoint(characterDetails[parseInt(i.toString(), 10)].Height));
    ...
    PdfRenderer.prototype.getDocumentTextCollection = function (value) {
        var pageStartIndex = !isNullOrUndefined(value.jsonObject.pageStartIndex) ? value.jsonObject.pageStartIndex : value.jsonObject.pageIndex;
        var pageEndIndex = !isNullOrUndefined(value.jsonObject.pageEndIndex) ? value.jsonObject.pageEndIndex : value.jsonObject.pageIndex + 1;
