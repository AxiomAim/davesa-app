    ...
    */
    ...
    FormDesigner.prototype.deleteFormField = function (formFieldId, addAction) {
        ...
        if (isNullOrUndefined(formField) && formFieldId) {
            ...
            var FormfieldsData = JSON.parse(data);
            this.pdfViewer.formFieldCollection = this.pdfViewer.formFieldCollection.filter(function (field) { return formFieldId.id !== field.id; });
            for (var i = 0; i < this.pdfViewer.formFieldCollections.length; i++) {
                if (formFieldId.id === this.pdfViewer.formFieldCollections[i].id) {
                    for (var j = 0; j < FormfieldsData.length; j++) {
                        if (formFieldId.name == FormfieldsData[j].FieldName) {
    ...
    */
    FormDesigner.prototype.setFormFieldIndex = function () {
        if (this.pdfViewer.formFieldCollections.length > 0) {
            ...
            var lastFormFieldIndex = lastFormField && lastFormField.name ? parseInt(lastFormField.name.match(/\d+/)) : null;
