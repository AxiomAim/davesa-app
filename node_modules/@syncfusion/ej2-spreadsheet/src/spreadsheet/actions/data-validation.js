...
import { EventHandler, remove, closest, isNullOrUndefined, select, Browser, Internationalization } from '@syncfusion/ej2-base';
        ...
        if (isNew) {
            ...
            if (type === 'Date') {
                var getFormattedDate = function (val) {
                    if (isNumber(val)) {
                        var args = { cell: { value: val }, type: 'date', value: val };
                        _this.parent.notify(getFormattedBarText, args);
                        return args.value;
                    }
                    return val;
                };
                value1 = getFormattedDate(value1);
                value2 = getFormattedDate(value2);
            }
            else {
                value1 = val1;
                value2 = val2;
            ...
            for (var idx = 0; idx < values.length; idx++) {
                if (checkIsFormula(values[idx])) {
                    ...
                    if (type === 'Date' || type === 'Time') {
                        value = this.getDateAsNumber({ range: [], cell: { value: values[idx] } }, values[idx]);
                        if (type === 'Date') {
                            values[idx] = value;
                        }
                    }
    ...
    DataValidation.prototype.isValidationHandler = function (args) {
        ...
        if (validation) {
            ...
            if (ignoreBlank && enterValue === '') {
                ...
                if (args.value && !isNumber(args.value)) {
                    if (isDateTimeType || validation.type === 'TextLength') {
                        ...
                        if (format) {
                            if (format !== '@') {
                                try {
                                    var colorCode = getColorCode(format);
                                    while (colorCode) {
                                    ...
                                    if (format.includes('%') && args.value.indexOf('%') > 0) {
                                        numVal = intl.getNumberParser({ format: 'n' })(args.value);
                            ...
                            var formatArgs = {
                                formattedText: args.value, value: args.value, format: 'General',
