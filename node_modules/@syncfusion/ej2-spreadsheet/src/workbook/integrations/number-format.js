import { getRangeIndexes, updateCell, applyCellFormat, currencyFormat } from '../common/index';
...
var WorkbookNumberFormat = /** @class */ (function () {
    function WorkbookNumberFormat(parent) {
        ...
        var curSym = getNumberDependable(this.parent.locale, option.currency);
        if (curSym !== '$') {
            ...
            if (formatStr && formatStr.endsWith('$')) {
                ...
                currencyFormat.currency.forEach(function (format, index) {
                    if (format.includes('$#,##0')) {
                        ...
                        currencyFormat.currency[index] = format.split("$#,##0" + decimalFormat).join("#,##0" + decimalFormat + curSpacing_1 + "$");
                    }
                });
                currencyFormat.accounting.forEach(function (format, index) {
                        ...
                        formatArr_1.forEach(function (formatStr, index) {
                            if (formatStr.includes('$')) {
                                ...
                                if (formatStr.includes('0)')) {
                                    ...
                                    replaceIdx_1 = formatStr.lastIndexOf(formatStr.includes('0') ? '0' : (formatStr.includes('?') ? '?' : (formatStr.includes('"-"') ? '"' : '#'))) + 1;
                                }
                                if (replaceIdx_1 > 0) {
                                    formatArr_1[index] = formatStr.slice(0, replaceIdx_1) + curSpacing_1 + '$' + formatStr.slice(replaceIdx_1);
                                }
                            }
                        });
                        currencyFormat.accounting[index] = formatArr_1.join(';');
                    }
                });
            }
        }
    ...
    WorkbookNumberFormat.prototype.getFormattedCell = function (args) {
        ...
        if (cell.format && this.isCustomType(cell)) {
            ...
            else if (isCustomDateTime(cell.format, true, option_1, true)) {
                if (fResult !== '') {
                    args.result = this.processCustomDateTime(args, cell, option_1.type !== 'time');
    ...
    WorkbookNumberFormat.prototype.processCustomDateTime = function (args, cell, isDate) {
        var _this = this;
        var isCustomDate;
    ...
    WorkbookNumberFormat.prototype.parseDecimalNumber = function (options) {
        ...
        if (cellValArr[0].length > 11) {
            ...
            if (options.cellVal.length > 11) {
                ...
                options.fResult = options.intl.formatNumber(Number(options.cellVal), { format: rightDigitLen > 0 ? "0." + '0'.repeat(rightDigitLen) : '0' });
                if (options.fResult) {
                    if (this.decimalSep === '.') {
    ...
    WorkbookNumberFormat.prototype.updateAutoDetectNumberFormat = function (options) {
        ...
        if (this.isPercentageValue(res, options.args, cell)) {
            ...
            var isEdit = this.decimalSep === '.' || (options.args.isEdit && !cell.formula);
    ...
    WorkbookNumberFormat.prototype.isPercentageValue = function (value, args, cell) {
        if (value.includes('%')) {
            ...
            if (valArr[0] !== '' && valArr[1].trim() === '') {
                var numArgs = checkIsNumberAndGetNumber({ value: valArr[0] }, this.parent.locale, this.groupSep, this.decimalSep);
                if (numArgs.isNumber) {
        ...
        if (numArgs.isNumber) {
            ...
            if (cellVal === 0) {
                ...
                if (!args.format.includes('#') && !args.format.includes('0')) {
                    args.format += '#';
                    var formattedText = intl.formatNumber(cellVal, { format: args.format, currency: currencyCode });
    ...
    WorkbookNumberFormat.prototype.checkDateFormat = function (args) {
        ...
        if (props.val !== 'Invalid') {
            var dateObj = toDate(props.val, args.intl || new Internationalization(this.parent.locale), this.parent.locale, props.format, args.skipCellFormat && cell, props.isDateTime);
