...
var Calculate = /** @class */ (function (_super) {
                ...
                var _loop_1 = function () {
                    ...
                    if (this_1.getLibraryFormulas().get(libFormula.toUpperCase()).isCustom) {
                        ...
                        if (nestedFormula && libFormula) {
                            var formulas = ['IF', 'INDEX', 'SORT', 'T', 'EXACT', 'PROPER', 'DOLLAR', 'DATE'];
    ...
    Calculate.prototype.computeVHLookup = function (range, vLookup) {
        var _a, _b;
        var lookupValue;
        var idxNumber;
        var lookupArray;
        ...
        var result = [];
        var argArr = range;
        var isVlookup = vLookup ? this.trueValue : this.falseValue;
        ...
        var idxNum = this.parseFloat(idxNumber);
        var rangeSplit = argArr[1].split(':');
        ...
        var lookupRangeArray = this.getCellCollection(lookupArray);
        var matchRangeArray = this.getCellCollection(matchArray);
        for (var i = 0; i < lookupRangeArray.length; i++) {
            findMaxVal.push(this.getValueFromArg(lookupRangeArray[i]).split(this.tic).join(''));
        }
        var criteriaVal = this.parseFloat(lookupValue);
        if (rangeLookup === 'FALSE') { // For EXACT Match
            for (var j = 0; j < lookupRangeArray.length; j++) {
                checkCriteria[j] = this.getValueFromArg(lookupRangeArray[j]).split(this.tic).join('');
                if (!isNullOrUndefined(matchRangeArray[j])) {
                    if (!this.isNaN(criteriaVal) && checkCriteria[j] !== '' && criteriaVal === this.parseFloat(checkCriteria[j])) {
                        result.push(this.getValueFromArg(matchRangeArray[j]).split(this.tic).join('') || '0');
                    }
                    else if (checkCriteria[j] !== '' && lookupValue.toUpperCase() === checkCriteria[j].toUpperCase()) {
                        result.push(this.getValueFromArg(matchRangeArray[j]).split(this.tic).join('') || '0');
                    }
                    else if (lookupValue.indexOf('*') > -1 || lookupValue.indexOf('?') > -1) {
                        var criteriaValue = lookupValue;
            ...
            var maxVal = num[num.length - 1];
            var minVal = num[0];
            var lookup = lookupValue.toUpperCase();
            for (var k = 0; k < lookupRangeArray.length; k++) {
                checkCriteria[k] = this.getValueFromArg(lookupRangeArray[k]).split(this.tic).join('');
                if (!isNullOrUndefined(matchRangeArray[k]) && lookupValue !== '' && checkCriteria[k] !== '' && (lookup === this.trueValue || lookup === this.falseValue) && lookup === checkCriteria[k].toUpperCase()) {
                    result.push(this.getValueFromArg(matchRangeArray[k]).split(this.tic).join('') || '0');
                }
                else if (!isNullOrUndefined(matchRangeArray[k]) && lookupValue !== '' && checkCriteria[k] !== '' && lookup === checkCriteria[k].toUpperCase()) {
                    result.push(this.getValueFromArg(matchRangeArray[k]).split(this.tic).join('') || '0');
                }
            }
            for (var a = 0; a < num.length; a++) {
                checkCriteria[a] = num[a].toString().split(this.tic).join('');
                if (!isNullOrUndefined(matchRangeArray[a]) && lookupValue !== '' && !this.isNaN(criteriaVal) && checkCriteria[a] !== '' && criteriaVal === this.parseFloat(checkCriteria[a])) {
                    result.push(this.getValueFromArg(matchRangeArray[a]).split(this.tic).join('') || '0');
                }
            }
            if (result.length > 0) {
                return result[result.length - 1];
            ...
            if (findMaxVal.indexOf(lookupValue.split(this.tic).join('')) < 0 && lookupValue !== this.trueValue && lookupValue !== this.falseValue) {
                if (!this.isNaN(criteriaVal) && lookupValue !== '' && !this.isNaN(maxVal) && !this.isNaN(minVal)) {
                    var temp = [];
                    for (var b = 0; b < num.length; b++) {
                        if (criteriaVal > num[b]) {
                            temp.push(num[b]);
                        }
                    }
                    var index = temp.length - 1;
                    if (!isNullOrUndefined(matchRangeArray[index]) && index >= 0) {
                        return this.getValueFromArg(matchRangeArray[index]).split(this.tic).join('') || '0';
                    }
                }
     ...
     */
    Calculate.prototype.getValueFromArg = function (arg, isUnique, isIfError, isSubtotal) {
        arg = arg.trim();
        var s = arg;
        ...
        if (isNullOrUndefined(s) || this.isTextEmpty(s)) {
            ...
            var parsedVal = arg.split(this.tic).join('');
            ...
            if (!this.isUpperChar(s[0]) && (this.isDigit(s[0]) || s[0] === decimalSep || s[0] === '-' || s[0] === 'n')) {
                ...
                if (decimalSep !== '.' && s.includes(decimalSep)) {
                    var parsedVal = s.replace(decimalSep, '.');
                    if (this.isNumber(parsedVal)) {
                        s = parsedVal;
                    }
                }
    ...
    Calculate.prototype.getParentObjectCellValue = function (val, refresh, isUnique, isSubtotal) {
        ...
        var cValue = this.getParentCellValue(row, col, this.grid, saveCell, grid, refresh, isUnique, isSubtotal);
        this.grid = grid;
        this.cell = saveCell;
        return cValue;
    };
    Calculate.prototype.getParentCellValue = function (row, col, grd, fromCell, fromCellGrd, refresh, isUnique, isSubtotal) {
        // formulainfotable
        var cValue;
        ...
        if (this.parentObject.getValueRowCol === undefined) {
            ...
            cValue = this.parentObject.getValueRowCol(gridId, row, col, fromCell, refresh, isUnique, isSubtotal);
        }
        if (isNullOrUndefined(cValue)) {
            ...
            if (decimalSep !== '.' && cValue.includes(decimalSep)) {
                var parsedVal = cValue.replace(decimalSep, '.');
                if (this.isNumber(parsedVal)) {
                    cValue = parsedVal;
                }
            }
