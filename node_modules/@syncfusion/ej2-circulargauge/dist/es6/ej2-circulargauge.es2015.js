import { Animation, Browser, ChildProperty, Collection, Complex, Component, Event, EventHandler, Internationalization, NotifyPropertyChanges, Property, SanitizeHtmlHelper, animationMode, compile, createElement, isNullOrUndefined, merge, print, remove, setStyleAttribute } from '@syncfusion/ej2-base';
...
function showTooltip(text, x, y, gauge, type) {
    let tooltipRect = new Rect(gauge.margin.left, gauge.margin.top + 10, gauge.availableSize.width - (gauge.margin.left + gauge.margin.right), gauge.availableSize.height - (gauge.margin.top - gauge.margin.bottom));
    let id;
    let tooltip;
    if (type === 'Title') {
        ...
        for (let i = 0, length = labelCollection.length; i < length; i++) {
            ...
            if (labelsVisible || (i === labelCollection.length - 1)) {
                ...
                label.text = (axis.showLastLabel && axis.hideIntersectingLabel && isCompleteAngle(axis.startAngle, axis.endAngle)
                    && (i === 0)) ? '' : label.text;
                const labelTextElement = textElement(new TextOption(gauge.element.id + '_Axis_' + index + '_Label_' + i, location.x, location.y, anchor, label.text, style.autoAngle ? 'rotate(' + (angle + 90) + ',' + (location.x) + ',' + location.y + ')' : '', 'auto'), textFont, style.useRangeColor ? getRangeColor(label.value, axis.ranges, color) : color, labelElement, 'pointer-events:none;');
...
class AxisLayoutPanel {
        ...
        if (element) {
            new Animation({}).animate(element, {
                ...
                // eslint-disable-next-line
                    ...
                    element.style.visibility = 'visible';
                    let axisElement = document.getElementById(this.gauge.element.id + '_Axis_Labels_' + axisIndex);
                    if (this.gauge.axes[axisIndex].showLastLabel && parseInt(element.id.split('Label_')[1]) === (axisElement.childElementCount - 2)) {
            ...
            new Animation({}).animate(tickElement, {
                ...
                progress: () => {
                    ...
                    if (axis.isMajorTick && axis.isMinorTick && gauge.allowLoadingAnimation && this.gauge.isAnimationProgress) {
                        ...
                        let minorTickInterval = (gauge.axes[axisIndex].minorTicks.interval != null ? gauge.axes[axisIndex].minorTicks.interval :
                        ...
                        const majorTickValue = this.prevAnimatedMajorTickValue[axisIndex] + gauge.axes[axisIndex].visibleRange.interval;
    ...
    rangeElementAnimation(rangeElement, axisIndex, gauge) {
        let height = 0;
        const opacity = 1;
        const isRangeAbsent = rangeElement.childElementCount > 0 && gauge.axes[axisIndex].ranges[0].start !== gauge.axes[axisIndex].ranges[0].end;
                ...
                if (gauge.allowLoadingAnimation && axisIndex === 0) {
                    ...
                    gauge.axes.map((axis, axisindex) => {
                        ...
                        if (axis.pointers.length === 0 && this.gauge.isAnimationProgress) {
                            if (this.gauge.loadingAnimationDuration[axisIndex] > 0 && !isNullOrUndefined(this.gauge.annotationsModule)) {
                                this.gauge.annotationsModule.annotationAnimation(this.gauge);
                            }
                        }
                        else {
...
class Legend {
    ...
    renderPagingElements(bounds, textOption, legendGroup) {
        ...
        const pageTextElement = textElement(textOption, legend.textStyle, grayColor, paginggroup);
    ...
    renderText(legendOption, group, textOptions, axisIndex, rangeIndex) {
        ...
        const legendTextElement = textElement(textOptions, legend.textStyle, fontcolor, group, '');
